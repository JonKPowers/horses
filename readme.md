...
during a horse race and the final time of that race. My hypothesis was that races run when it is very hot would be relatively
...
1,435 columns, all of which is contained in a single file type. Because each file has a different columns structure 
...
of the other file types that can be processed. These other files contain time and distance information that could be 
...
Next, `main()` begins processing files. By default it will process all files contained in the `/data` directory other
that files having an unsupported extension (which are skipped and left alone). If it appears that a file has
...
This was particularly prevalent in items ares like medications, racing equipment, and age/sex restrictions. For
